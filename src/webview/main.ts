import {
    ...
    vsCodeTextArea,
    DataGrid,
    DataGridCell } from "@vscode/webview-ui-toolkit";

import mermaid from 'mermaid';

provideVSCodeDesignSystem().register(vsCodeButton(), vsCodeDataGrid(), vsCodeDataGridCell(), vsCodeDataGridRow(), vsCodeTextArea());
...
function handleSaveAsPng() {
    ...
    svgImage.onload = function () {
        ...
        a.setAttribute("download", flowMap['label'].replaceAll(' ', '-').toLowerCase() + ".flow.png");
        a.dispatchEvent(new MouseEvent("click"));
    };
    const svg = document.querySelector("svg");
    const svgData = new XMLSerializer().serializeToString(svg as SVGSVGElement);
    svgImage.src = "data:image/svg+xml;charset=utf-8;base64," + btoa(unescape(encodeURIComponent(svgData))); 
 }
